...
function applySnapshot(snapshot) {
  ...
  // localChangedAt must be stamped to match cloudUpdatedAt here. Otherwise, on
  // every future load, cloudChangedAt > localChangedAt keeps evaluating true
  // (localChangedAt would stay 0 forever on a device that only ever pulls),
  // and handleAuthState reloads the page again... which pulls again...
  // which reloads again, forever. This was the cause of the endless reload loop.
  const stamp = Number(snapshot.updatedAt || Date.now());
  localStorage.setItem(ADDON_KEYS.syncMeta, JSON.stringify({ pulledAt: Date.now(), cloudUpdatedAt: stamp, localChangedAt: stamp }));
  applyingCloud = false;
}
...
async function handleAuthState(user) {
  ...
  try {
    ...
    // Safety net: only ever auto-reload the page once per tab session. If a
    // future bug reintroduces a mismatch that looks "newer forever", this
    // stops it from looping instead of just stopping this one known cause.
    const alreadyReloadedThisSession = sessionStorage.getItem("planner_cloud_reloaded") === "1";
    if (!localHasPlannerData()) {
      applySnapshot(cloud);
      if (alreadyReloadedThisSession) return;
      sessionStorage.setItem("planner_cloud_reloaded", "1");
      location.reload();
      return;
    }
  ...
  section.insertAdjacentHTML("beforeend", `
    <div class="tracker-pane">
      ...
      <form class="addon-form" id="studyForm">
        ...
        <input class="addon-field" id="studyMinutes" type="number" min="1" max="1440" required placeholder="minutes">
        <input class="addon-field wide" id="studyNotes" maxlength="120" placeholder="optional notes">
        <button class="btn blue sm">+ log</button>
      </form>
